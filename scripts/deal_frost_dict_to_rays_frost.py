...
file_list = []
# 获取当前目录下的所有文件名：
for file_name in os.listdir('frost/cn_dicts'):
    # 判断文件名是否以'.yaml'结尾
...
dict_data['moqi'] = get_aux_code_map([resources/'moqi_chaifen.txt', resources/'moqi_chaifen_rongcuo.txt'])
...
dict_data['hm_full4'] = get_hu_full4_aux_code_map([resources/'hu_cf.txt'])
dict_data['hm'] = get_hu_aux_code_map([resources/'hu_cf.txt'])
...
for file_name in file_list:
    # File paths
    yaml_file_path = Path('frost/cn_dicts', file_name)
    write_file_path = Path('frost/cn_dicts_ray', file_name)
...
file_list = []
# 获取当前目录下的所有文件名：
for file_name in os.listdir('frost/cn_dicts_cell'):
    # 判断文件名是否以'.yaml'结尾
    if file_name.endswith('.yaml'):
        file_list.append(file_name)
print("文件列表：", file_list)
...
for file_name in file_list:
    # File paths
    yaml_file_path = Path('frost/cn_dicts_cell', file_name)
    write_file_path = Path('frost/cn_dicts_ray_cell', file_name)
