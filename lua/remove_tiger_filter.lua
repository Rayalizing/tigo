...
function M.func(input, env)
    ...
    -- 🐯 虎句开关
        ...
        for _, cand in ipairs(tiger_sentence) do
            ...
            if 2 * utf8.len(cand.text) <=  letter_count then
                table.insert(first_candidates, cand)
            else
                table.insert(second_candidates, cand)
            end
        end    
        
        if context:get_option("simple") then
            for _, cand in ipairs(second_candidates) do
                yield(cand)
            end      
            for _, cand in ipairs(first_candidates) do
                yield(cand)
            end
            else
            for _, cand in ipairs(first_candidates) do
